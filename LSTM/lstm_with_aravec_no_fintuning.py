...
def run_kfold_best_params(all_texts, all_labels, vocab, embedding_tensor,
    ...
    # ── Graphique 1: toutes les époques (sans suffixe, titres simples) ──
    _plot_kfold_metrics(df_agg, df_agg, K_FOLDS,
                        save_path=f"{results_dir}/kfold_metriques_complet_{timestamp}.png")

    # ── Graphique 2: jusqu'à l'époque optimale (titres simples) ──
...
def plot_final_metrics(history, save_path):
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - LSTM avec AraVec gelés',
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Matrice de confusion - LSTM avec AraVec gelés',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def save_vectorization(vocab, embedding_tensor, label_encoder, aravec_obj,
    ...
    info = {
        ...
        'plongements_entrainables': False,   # Gelés
    ...
    print("NOTE: Les plongements sont GELÉS (pas de fine-tuning)")
    print("=" * 70)
...
def main():
    ...
    if X_test and len(y_test) > 0:
        ...
        print(f"  Plongements:          GELÉS (pas de fine-tuning)")
