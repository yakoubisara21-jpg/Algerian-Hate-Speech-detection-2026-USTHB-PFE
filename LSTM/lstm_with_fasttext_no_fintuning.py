"""
LSTM avec plongements FastText (NON entraînables / gelés) pour la classification de texte - Version finale
...
class LSTMClassifier(nn.Module):
    """
    Classifieur LSTM unidirectionnel avec plongements FastText NON ENTRAÎNABLES (gelés).
...
def plot_final_metrics(history, save_path):
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - LSTM avec FastText gelés',
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Matrice de confusion - LSTM avec FastText gelés',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def run_optuna(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print("LSTM UNIDIRECTIONNEL - PLONGEMENTS NON ENTRAÎNABLES (gelés / frozen)")
...
def main():
    ...
    print("=" * 70)
    print("LSTM avec plongements FastText (NON entraînables / gelés) - Classification de texte avec OPTUNA")
    print("OBJECTIF OPTUNA: MAXIMISATION DU F1 DE VALIDATION")
    print("ÉPOQUE OPTIMALE MODÈLE FINAL: MIN PERTE VALIDATION (K-FOLD)")
    print(f"VALIDATION CROISÉE STRATIFIÉE K-FOLDS K={K_FOLDS}")
    print("LSTM UNIDIRECTIONNEL - PLONGEMENTS NON ENTRAÎNABLES (gelés / frozen)")
