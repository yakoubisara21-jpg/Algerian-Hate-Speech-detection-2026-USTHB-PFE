"""
Complete LSTM with FastText Multilingual Embeddings for Text Classification - Full Version with ROC
...
✅ FORCED UNIDIRECTIONAL LSTM (No BiLSTM)
✅ FROZEN EMBEDDINGS (Prevents overfitting)
✅ FIXED EPOCHS = 25 (No early stopping - runs all epochs)
✅ STANDARD BCE LOSS (No label smoothing)
...
def build_vocabulary(texts, min_freq=3):
    """Build vocabulary from texts - HIGHER min_freq reduces overfitting"""
    ...
    print(f"Vocabulary size: {len(vocab)} (filtered with min_freq={min_freq})")
    return vocab
...
# ============================================================================
# PART 3: LSTM MODEL (FORCED UNIDIRECTIONAL - NO BIDIRECTIONAL)
# ============================================================================

class LSTMTextClassifier(nn.Module):
    """LSTM-based text classifier with FastText embeddings - FORCED UNIDIRECTIONAL LSTM"""
    
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim, 
                 n_layers=2, 
                 dropout_pre_lstm=0.3, dropout_lstm=0.5, 
                 dropout_post_lstm=0.3,
                 pretrained_embeddings=None):
        super().__init__()
        
        self.hidden_dim = hidden_dim
        self.n_layers = n_layers
        self.bidirectional = False  # FORCED UNIDIRECTIONAL
        
        self.embedding = nn.Embedding(vocab_size, embedding_dim, padding_idx=0)
        self.dropout_pre_lstm = nn.Dropout(dropout_pre_lstm)
        ...
        if pretrained_embeddings is not None:
            ...
            print("🔒 FastText embeddings FROZEN (prevents overfitting)")
        ...
        self.lstm = nn.LSTM(
            ...
            num_layers=n_layers,
            bidirectional=False,
            dropout=dropout_lstm if n_layers > 1 else 0,
            batch_first=True
        )
        
        lstm_output_dim = hidden_dim
        self.dropout_post_lstm = nn.Dropout(dropout_post_lstm)
        self.fc = nn.Linear(lstm_output_dim, output_dim)
        self.sigmoid = nn.Sigmoid()
        
    def forward(self, text, text_lengths):
        embedded = self.embedding(text)
        embedded = self.dropout_pre_lstm(embedded)
        ...
        hidden = self.dropout_post_lstm(hidden)
...
# ============================================================================
# PART 4: TRAINING FUNCTIONS WITH COMPLETE METRICS (STANDARD BCE LOSS)
# ============================================================================
...
def train_model_with_all_metrics(model, train_loader, val_loader, epochs, lr, weight_decay, scheduler_factor=0.5, scheduler_patience=3):
    ...
    os.makedirs('DL Models/LSTM with FastText', exist_ok=True)
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'val_precision': val_prec,
                'val_recall': val_rec
            }, 'DL Models/LSTM with FastText/best_lstm_model.pth')
...
def objective(trial):
    ...
    dropout_pre_lstm = trial.suggest_float('dropout_pre_lstm', 0.2, 0.6, step=0.1)
    dropout_lstm = trial.suggest_float('dropout_lstm', 0.3, 0.7, step=0.1)
    dropout_post_lstm = trial.suggest_float('dropout_post_lstm', 0.2, 0.6, step=0.1)
    ...
    model = LSTMTextClassifier(
        ...
        output_dim=1,
        n_layers=n_layers,
        dropout_pre_lstm=dropout_pre_lstm,
        dropout_lstm=dropout_lstm,
        dropout_post_lstm=dropout_post_lstm,
...
def run_optuna_optimization(train_loader, val_loader, vocab_size, embedding_matrix, n_trials=30):
    ...
    print("✅ FORCED UNIDIRECTIONAL LSTM (No BiLSTM)")
    print("✅ FROZEN EMBEDDINGS")
    print("✅ STANDARD BCE LOSS (No label smoothing)")
    print("✅ REDUCELROnPLATEAU scheduler")
    print("✅ FIXED 25 EPOCHS (No early stopping)")
    print("="*70)
    
    study = optuna.create_study(
        direction='minimize',
        study_name='lstm_text_classification_loss',
        storage=None,
    ...
    results_dir = 'DL Models/LSTM with FastText/Results'
...
# ============================================================================
# PART 6: METRICS PLOTTING (Normal - shows ALL epochs, auto-scale y-axis)
# ============================================================================

def plot_metrics(all_epoch_history, save_path):
    """
    Plot ALL metrics - normal plotting, all epochs shown, auto-scale y-axis
    """
    ...
    # Create figure with 2 rows, 3 columns (but only use 5 subplots)
    fig, axes = plt.subplots(2, 3, figsize=(18, 10))
    axes = axes.flatten()
    
    # 1. Loss Curve
    axes[0].plot(epochs, train_loss, 'b-o', linewidth=2, markersize=6, label='Training')
    axes[0].plot(epochs, val_loss, 'r-s', linewidth=2.5, markersize=8, label='Validation')
    ...
    # 2. Accuracy Curve
    axes[1].plot(epochs, train_acc, 'b-o', linewidth=2, markersize=6, label='Training')
    axes[1].plot(epochs, val_acc, 'r-s', linewidth=2.5, markersize=8, label='Validation')
    ...
    # 3. F1 Score Curve
    axes[2].plot(epochs, train_f1, 'b-o', linewidth=2, markersize=6, label='Training')
    axes[2].plot(epochs, val_f1, 'r-s', linewidth=2.5, markersize=8, label='Validation')
    ...
    # 4. Precision Curve
    axes[3].plot(epochs, train_prec, 'b-o', linewidth=2, markersize=6, label='Training')
    axes[3].plot(epochs, val_prec, 'r-s', linewidth=2.5, markersize=8, label='Validation')
    ...
    # 5. Recall Curve
    axes[4].plot(epochs, train_rec, 'b-o', linewidth=2, markersize=6, label='Training')
    axes[4].plot(epochs, val_rec, 'r-s', linewidth=2.5, markersize=8, label='Validation')
    ...
    # 6. Hide the 6th subplot (no learning rate graph)
    axes[5].set_visible(False)
    
    plt.suptitle('Training and Validation Metrics - LSTM with FastText Embeddings', 
...
def plot_roc_curve(true_labels, probabilities, label_encoder, save_path):
    ...
    ax.set_title('Receiver Operating Characteristic (ROC) Curve - LSTM with FastText', 
...
def plot_confusion_matrix_graph(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Confusion Matrix - LSTM with FastText', fontsize=14, fontweight='bold')
...
def train_final_model_with_best_params(train_loader, val_loader, vocab_size, embedding_matrix, best_params):
    ...
    model = LSTMTextClassifier(
        ...
        dropout_pre_lstm=best_params['dropout_pre_lstm'],
        dropout_lstm=best_params['dropout_lstm'],
        dropout_post_lstm=best_params['dropout_post_lstm'],
    ...
    # Force 25 epochs for final training
    ...
    if os.path.exists('DL Models/LSTM with FastText/best_lstm_model.pth'):
        checkpoint = torch.load('DL Models/LSTM with FastText/best_lstm_model.pth')
...
def main():
    ...
    print("="*70)
    print("LSTM with FastText Arabic Embeddings - Text Classification with OPTUNA")
    print("OPTIMIZING FOR: VALIDATION LOSS (MINIMIZATION)")
    print("✅ FORCED UNIDIRECTIONAL LSTM (No BiLSTM)")
    print("✅ FROZEN EMBEDDINGS")
    print("✅ STANDARD BCE LOSS (No label smoothing)")
    print("✅ REDUCELROnPLATEAU scheduler")
    print("✅ HIGHER MIN_FREQ (3)")
    print("✅ FIXED 25 EPOCHS (No early stopping)")
    print("="*70)
    ...
    output_dir = 'DL Models/LSTM with FastText'
    ...
    all_texts = X_train
    vocab = build_vocabulary(all_texts, min_freq=3)
    ...
    val_loss, val_acc, val_prec, val_rec, val_f1, val_preds, val_labels, val_probabilities = evaluate_with_all_metrics(
        trained_model, val_loader_opt, eval_criterion, threshold=0.5
    )
    ...
    test_loss, test_acc, test_prec, test_rec, test_f1, predictions, true_labels, probabilities = evaluate_with_all_metrics(
        trained_model, test_loader, eval_criterion, threshold=optimal_threshold
    )
    ...
    perm_results = permutation_test_accuracy(
        trained_model, test_loader, eval_criterion,
