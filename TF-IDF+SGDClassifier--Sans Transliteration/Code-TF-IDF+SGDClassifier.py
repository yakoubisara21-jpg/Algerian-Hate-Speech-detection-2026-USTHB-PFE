...
print(f"Chargement de validation.csv depuis: {VAL_PATH}")
if not os.path.exists(VAL_PATH):
    raise FileNotFoundError(f"Fichier manquant: {VAL_PATH}")

df_train = pd.read_csv(TRAIN_PATH, sep=';').dropna()
df_val   = pd.read_csv(VAL_PATH,   sep=';').dropna()

df_kfold = pd.concat([df_train, df_val], ignore_index=True)
X_kfold  = df_kfold['text'].astype(str)
y_kfold  = df_kfold['label'].astype(int)

print(f"  train.csv    : {len(df_train)} échantillons")
print(f"  validation.csv: {len(df_val)} échantillons")
print(f"  Combiné (CV) : {len(df_kfold)} échantillons")
...
joblib.dump(tfidf, os.path.join(RESULTS_DIR, f'vectorizer_{timestamp}.pkl'))
print(f"Vectoriseur sauvegardé: vectorizer_{timestamp}.pkl")
...
pd.DataFrame([best_params]).to_csv(
    os.path.join(RESULTS_DIR, f'meilleurs_hyperparametres_{timestamp}.csv'), index=False)
...
for fold_idx, (train_idx, val_idx) in enumerate(
    ...
    for epoch in range(N_EPOCHS):
        ...
        fold_metrics['train']['f1'][fold_idx, epoch]         = f1_score(y_tr, y_tr_pred, average='macro', zero_division=0)
        fold_metrics['train']['precision'][fold_idx, epoch]  = precision_score(y_tr, y_tr_pred, average='macro', zero_division=0)
        fold_metrics['train']['recall'][fold_idx, epoch]     = recall_score(y_tr, y_tr_pred, average='macro', zero_division=0)
        ...
        fold_metrics['val']['f1'][fold_idx, epoch]           = f1_score(y_vl, y_vl_pred, average='macro', zero_division=0)
        fold_metrics['val']['precision'][fold_idx, epoch]    = precision_score(y_vl, y_vl_pred, average='macro', zero_division=0)
        fold_metrics['val']['recall'][fold_idx, epoch]       = recall_score(y_vl, y_vl_pred, average='macro', zero_division=0)
...
for epoch in range(OPTIMAL_EPOCHS):
    ...
    tr_f1     = f1_score(y_kfold.values, y_tr_pred, average='macro', zero_division=0)
    tr_prec   = precision_score(y_kfold.values, y_tr_pred, average='macro', zero_division=0)
    tr_rec    = recall_score(y_kfold.values, y_tr_pred, average='macro', zero_division=0)
...
log_loss_test  = log_loss(y_test, np.column_stack([1 - y_prob, y_prob]))
...
joblib.dump(final_model, os.path.join(RESULTS_DIR, f'SGDClassifier_model_{timestamp}.pkl'))
model_artifacts = {
...
with open(os.path.join(RESULTS_DIR, f'SGDClassifier_artifacts_{timestamp}.pkl'), 'wb') as f:
...
sample_res = predict_comment(
    X_test.iloc[0],
    os.path.join(RESULTS_DIR, f'SGDClassifier_artifacts_{timestamp}.pkl')
)
