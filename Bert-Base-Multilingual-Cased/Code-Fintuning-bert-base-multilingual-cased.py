...
def _plot_kfold_metrics(df_plot, k_folds, title_suffix, save_path):
    ...
    for i, (train_col, val_col, ylabel, title_label) in enumerate(metrics_config):
        ...
        if val_col is None:
            # pour AUC, tracer seulement la val_auc_mean
            val_only_col = 'val_auc_mean'
            if val_only_col in df_plot.columns:
                ax.plot(epochs, df_plot[val_only_col], 'r-s', label='Validation', linewidth=2)
    ...
    plt.suptitle(f'Métriques de la validation croisée{title_suffix} — Bert-Base-Multilingual-Cased fine-tuné')
    plt.tight_layout()
...
def plot_final_metrics(history, save_path):
    ...
    plt.suptitle("Métriques d'entraînement final — Bert-Base-Multilingual-Cased fine-tuné")
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path, save_dir, timestamp):
    ...
    ax.set_title("Temps d'exécution — Bert-Base-Multilingual-Cased fine-tuné")
...
def compute_and_plot_test_auc(true_labels, probabilities, label_encoder, results_dir, timestamp):
    ...
    if num_classes == 2:
        ...
        ax.set_title('Courbe ROC — Bert-Base-Multilingual-Cased fine-tuné')
        ax.legend()
        ax.grid(alpha=0.3)
        plt.tight_layout()
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_ylabel('Réel')
    ax.set_title('Matrice de confusion — Bert-Base-Multilingual-Cased fine-tuné')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def permutation_test(model, tokenizer, test_texts, test_labels, label_encoder,
    ...
    ax.set_title('Test de permutation — Bert-Base-Multilingual-Cased fine-tuné')
    ax.legend()
    ax.grid(alpha=0.3)
    plt.tight_layout()
...
def main():
    ...
    print("="*70)
    print("Bert-Base-Multilingual-Cased Fine-tuning")
