"""
FAD-GRU: Fusion Multi-Embedding avec GRU (FastText + AraVec + DziriBERT)
...
MODEL_NAME = "Fusion Multi-Embedding avec GRU (FAD-GRU)"
MODEL_NAME_SHORT = "FAD-GRU"
...
class HybridGRUClassifier(nn.Module):
    ...
    FastText(300) ─┐
    AraVec(300)  ──┼─ cat(1368) ─► MLP(1368→768→512) ─► BiGRU ─► Pooling ─► Classifier
    DziriBERT(768)─┘
    ...
    def forward(self, token_ids, seq_lens, dziri_embs):
        ...
        # Mean pooling sur les tokens réels
...
def _plot_kfold_metrics(df, save_path):
    ...
    plt.suptitle(f'Métriques K-Fold — {MODEL_NAME}',
...
def evaluate_on_test(model, test_texts, test_labels, vocab, dziribert_cache,
    ...
    metrics = {
        'perte': te_loss, 'exactitude': te_acc, 'precision': te_prec,
        'rappel': te_rec, 'f1_score': te_f1, 'auc': te_auc
    }
    pd.DataFrame([metrics]).to_csv(f"{results_dir}/rapport_metriques_{timestamp}.csv", index=False)
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path):
    ...
    ax.set_title('Temps d\'exécution - FAD-GRU', fontsize=14, fontweight='bold')
