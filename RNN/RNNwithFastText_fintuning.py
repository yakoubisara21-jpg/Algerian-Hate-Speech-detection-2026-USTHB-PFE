...
def optuna_objective(trial):
    ...
    learning_rate      = trial.suggest_float('learning_rate',      1e-4,  5e-3, log=True)  # Légèrement augmenté pour fine-tuning
...
def run_kfold_best_params(all_texts, all_labels, vocab, embedding_tensor,
    ...
    print("NOTE: Plongements FastText ENTRAÎNABLES (fine-tuning)")
...
def _plot_kfold_metrics(df_full, df_plot, k_folds, title_suffix, save_path):
    ...
    plt.suptitle('Métriques validation croisée pour RNN avec FastText (fine-tuning)',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def plot_final_metrics(history, save_path):
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - RNN avec FastText (fine-tuning)',
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path):
    ...
    colors = ['#E65100', '#1565C0']  # Orange pour fine-tuning
    ...
    ax.set_title('Temps d\'exécution (fine-tuning actif)', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Matrice de confusion - RNN avec FastText (fine-tuning)',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def save_vectorization(vocab, embedding_tensor, label_encoder, fasttext_obj,
    ...
    emb_meta = {
        ...
        'entrainable':  True  # Plongements entraînables (fine-tuning)
    }
    ...
    info = {
        ...
        'plongements_entrainables': True,  # Fine-tuning actif
