...
EPOCHES FIXES = 20 (Pas d'arrêt précoce pour la recherche d'hyperparamètres)
...
GRAPHES SÉPARÉS: 20 ÉPOQUES COMPLET + GRAPHE JUSQU'À L'ÉPOQUE OPTIMALE (SANS AUC)
...
# ── Constantes ──
FIXED_EPOCHS = 20          # Nombre fixe d'époques pour la recherche
...
def run_kfold_best_params(all_texts, all_labels, vocab, embedding_tensor,
    ...
    # ── Graphique 1: toutes les époques (5 métriques, sans AUC) ──
    _plot_kfold_metrics(df_agg, df_agg, K_FOLDS,
                        title_suffix="- Toutes époques (sans AUC)",
                        save_path=f"{results_dir}/kfold_metriques_complet_{timestamp}.png")

    # ── Graphique 2: jusqu'à l'époque optimale (sans AUC) ──
    _plot_kfold_metrics(df_agg, df_agg.iloc[:optimal_epoch], K_FOLDS,
                        title_suffix=f"- Jusqu'à l'époque optimale {optimal_epoch}",
...
def _plot_kfold_metrics(df_full, df_plot, k_folds, title_suffix, save_path):
    ...
    plt.suptitle(f'Métriques validation croisée ({k_folds} plis) {title_suffix}',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def plot_final_metrics(history, save_path):
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - RNN avec AraVec (plongements gelés)',
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Matrice de confusion - RNN avec AraVec (plongements gelés)',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
