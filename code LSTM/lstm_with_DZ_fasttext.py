"""
LSTM avec plongements FastText (entraînables) pour la classification de texte - Version finale
...
def _plot_kfold_metrics(df_full, df_plot, k_folds, save_path):
    ...
    plt.suptitle('Métriques validation croisée pour LSTM avec DZ FastText',
...
def plot_roc_test(true_labels, probabilities, save_path):
    ...
    plt.title('ROC Curve - LSTM avec DZ FastText', fontsize=14, fontweight='bold')
...
def plot_final_metrics(history, save_path):
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - LSTM avec DZ FastText',
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path):
    ...
    ax.set_title('Temps d\'exécution - LSTM avec DZ FastText', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Matrice de confusion - LSTM avec DZ FastText ',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def run_optuna(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print("LSTM UNIDIRECTIONNEL - PLONGEMENTS ENTRAÎNABLES (FastText)")
...
def main():
    ...
    print("=" * 70)
    print("LSTM avec plongements FastText (entraînables) - Classification de texte avec OPTUNA")
    print("=" * 70)
