"""
LSTM avec plongements AraVec (GELÉS - Pas de fine-tuning) pour la classification de texte - Version finale
...
SCHEDULER SUR LA PERTE de valiation
...
class LSTMClassifier(nn.Module):
    """
    Classifieur LSTM unidirectionnel avec plongements AraVec GELÉS (pas de fine-tuning).
...
def train_with_history(model, train_loader, val_loader, epochs, lr, weight_decay,
    ...
    Retourne: historique, probabilités_val_par_époque, époques_de_réduction
    """
...
def _plot_kfold_metrics(df_full, df_plot, k_folds, save_path):
    ...
    plt.suptitle('Métriques validation croisée pour LSTM avec AraVec',
...
def plot_roc_test(true_labels, probabilities, save_path):
    ...
    plt.title('ROC Curve - LSTM avec AraVec', fontsize=14, fontweight='bold')
...
def plot_final_metrics(history, save_path):
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - LSTM avec AraVec',
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path):
    ...
    ax.set_title('Temps d\'exécution - LSTM avec AraVec', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Matrice de confusion - LSTM avec AraVec',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def save_vectorization(vocab, embedding_tensor, label_encoder, aravec_obj,
                        save_dir='DL Models/LSTM with AraVec/vectorisation'):
...
def run_optuna(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print("LSTM UNIDIRECTIONNEL - PLONGEMENTS GELÉS (pas de fine-tuning)")
    ...
    study = optuna.create_study(
        direction='maximize',
        study_name='lstm_aravec_f1_maximisation',
        sampler=sampler
    )
    ...
    results_dir = 'DL Models/LSTM with AraVec/Résultats'
...
def main():
    ...
    print("=" * 70)
    print("LSTM avec plongements AraVec (GELÉS) - Classification de texte avec OPTUNA")
    print("=" * 70)
    ...
    output_dir  = 'DL Models/LSTM with AraVec'
