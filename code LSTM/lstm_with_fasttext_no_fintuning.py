"""
LSTM avec plongements FastText (non entraînables) pour la classification de texte - Version finale
...
PLONGEMENTS NON ENTRAÎNABLES (FastText)
...
ROC SUR TEST.CSV UNIQUEMENT (TITRE: ROC Curve)
GRAPHIQUE DE TEMPS: UN SEUL GRAPHE AVEC DEUX BARRES (ENTRAÎNEMENT FINAL + TOTAL)
LSTM UNIDIRECTIONNEL (FORCÉ) - PAS DE BIDIRECTIONNEL
PLONGEMENTS FASTTEXT ARABES (NON ENTRAÎNABLES)
"""
...
class LSTMClassifier(nn.Module):
    """
    Classifieur LSTM unidirectionnel avec plongements FastText ENTRAÎNABLES.
    Le bidirectionnel est désactivé de force.
    """

    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim,
                 n_layers=2,
                 dropout_pre_lstm=0.3,
                 dropout_lstm=0.5,
        ...
        if pretrained_embeddings is not None:
            ...
            print("Plongements FastText GELÉS (fine-tuning désactivé)")
    ...
    def forward(self, text, text_lengths):
        # Plongements + dropout
        embedded = self.dropout_pre_lstm(self.embedding(text))
        ...
        # Dernière couche cachée de la dernière couche LSTM
        out = self.dropout_post_lstm(hidden[-1, :, :])
...
def train_with_history(model, train_loader, val_loader, epochs, lr, weight_decay,
    ...
    Le scheduler ReduceLROnPlateau surveille la PERTE De validation.
    """
    ...
    for epoch in range(epochs):
        ...
        scheduler.step(val_loss)  # ← now monitors val_loss
...
def run_kfold_best_params(fold_histories, results_dir, timestamp):
    ...
    for metric in metrics_names:
        epoch_vals = [[fold_histories[p][e][metric] for p in range(K_FOLDS)]
                      for e in range(FIXED_EPOCHS)]
    ...
    optimal_epoch_idx = int(np.argmin(df_agg['val_loss_moy']))
    optimal_epoch     = optimal_epoch_idx + 1
    print(f"Époque optimale (min val_loss): {optimal_epoch}")

    auc_at_optimal = [fold_histories[p][optimal_epoch_idx]['val_auc'] for p in range(K_FOLDS)]
    mean_auc = float(np.mean(auc_at_optimal))
...
def _plot_kfold_metrics(df_full, df_plot, k_folds, save_path):
    ...
    plt.suptitle('Métriques validation croisée pour LSTM avec FastText',
...
def plot_roc_test(true_labels, probabilities, save_path):
    ...
    plt.title('ROC Curve - LSTM avec FastText', fontsize=14, fontweight='bold')
    ...
    pd.DataFrame({'fpr': fpr, 'tpr': tpr}).to_csv(save_path.replace('.png', '_data.csv'), index=False)
...
def plot_final_metrics(history, save_path):
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - LSTM avec FastText ',
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path):
    ...
    ax.set_title('Temps d\'exécution - LSTM avec FastText', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Matrice de confusion - LSTM avec FastText ',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def save_vectorization(vocab, embedding_tensor, label_encoder, fasttext_obj,
                        save_dir='DL Models/LSTM with FastText/vectorisation'):
...
def run_optuna(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print("LSTM UNIDIRECTIONNEL - PLONGEMENTS NON ENTRAÎNABLES ")
    ...
    study = optuna.create_study(
        direction='maximize',
        study_name='lstm_fasttext_f1_maximisation',
        sampler=sampler
    )
    ...
    results_dir = 'DL Models/LSTM with FastText/Résultats'
...
def main():
    ...
    print("=" * 70)
    print("LSTM avec plongements FastText (non entraînables) - Classification de texte avec OPTUNA")
    print("=" * 70)
    print("⚠️  COURBE ROC UNIQUEMENT SUR TEST.CSV (TITRE: ROC Curve) ⚠️")
    print("=" * 70)
    ...
    best_params, best_factor, best_patience, study = run_optuna(
        X_kfold, y_kfold, vocab, embedding_tensor, n_trials=15, n_startup_trials=5)
    ...
    print(f"\nNombre optimal d'époques (min perte_val): {optimal_epochs}")
    ...
    if X_test and len(y_test) > 0:
        ...
        print(f"\n📈 COURBE ROC générée sur test.csv (titre: ROC Curve)")
        print("=" * 70)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if df_results is not None and test_results:
            ...
            print(f"✅ ROC Curve sauvegardée dans le dossier Résultats")
