...
perm_df.to_csv(
    os.path.join(RESULTS_DIR, f'test_permutation_{timestamp}.csv'), index=False)
...
ax.set_title(f'Test de Permutation — SGDClassifier avec TF-IDF\n'
             f'p-valeur = {p_value:.4f} ({sig_label})',
             fontsize=13, fontweight='bold')
ax.legend(fontsize=10)
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, f'test_permutation_{timestamp}.png'),
...
runtime_info = {
    ...
    'arret_anticipe':                 'non',
    'p_valeur_permutation': p_value,
    'conclusion_permutation': sig_label,
}
