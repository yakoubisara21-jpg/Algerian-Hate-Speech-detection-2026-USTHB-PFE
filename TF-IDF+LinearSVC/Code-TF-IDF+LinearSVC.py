...
print(f"Exactitude observée: {observed_accuracy:.4f}")
...
for i in range(n_permutations):
    ...
    perm_scores[i] = accuracy_score(y_perm, y_pred)
    if (i + 1) % 100 == 0:
        print(f"  Permutations: {i+1}/{n_permutations}")

p_value = np.mean(perm_scores >= observed_accuracy)

if p_value < 0.001:
    sig_label = "*** hautement significatif"
elif p_value < 0.01:
    sig_label = "** très significatif"
elif p_value < 0.05:
    sig_label = "* significatif"
else:
    sig_label = "non significatif"

print(f"\np-valeur: {p_value:.4f} - {sig_label}")

# Save permutation results
perm_df = pd.DataFrame({
    'n_permutations':      [n_permutations],
    'exactitude_observee': [observed_accuracy],
    'p_valeur':            [p_value],
    'conclusion':          [sig_label],
})
perm_df.to_csv(f'test_permutation_{timestamp}.csv', index=False)
print(f"Résultats sauvegardés → test_permutation_{timestamp}.csv")

# Plot permutation distribution
fig, ax = plt.subplots(figsize=(8, 5))
ax.hist(perm_scores, bins=30, color=COLORS['aux3'], alpha=0.7,
        edgecolor='white', label='Exactitudes permutées')
ax.axvline(observed_accuracy, color=COLORS['aux2'], linewidth=2.5,
           linestyle='--', label=f'Observée = {observed_accuracy:.4f}')
ax.set_xlabel('Exactitude', fontsize=12)
ax.set_ylabel('Fréquence', fontsize=12)
ax.set_title(f'Test de Permutation — LinearSVC avec TF-IDF\n'
             f'p-valeur = {p_value:.4f} ({sig_label})',
             fontsize=13, fontweight='bold')
ax.legend(fontsize=10)
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f'test_permutation_{timestamp}.png', dpi=300, bbox_inches='tight')
plt.close()
print(f"Graphique sauvegardé → test_permutation_{timestamp}.png")
...
def plot_learning_curves_svc(estimator, X, y, cv_folds=5,
    ...
    fig.suptitle(
        'Métriques validation croisée — Courbes d\'Apprentissage pour LinearSVC avec TF-IDF\n',
...
for m, ts, tm in zip(time_metrics['Métrique'],
                     ...
                     time_metrics['Temps (min)']):
    print(f"  {m}: {ts:.2f} sec ({tm:.2f} min)")
print(f"\np-valeur permutation: {p_value:.4f} ({sig_label})")
