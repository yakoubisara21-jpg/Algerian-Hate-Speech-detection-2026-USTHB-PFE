...
results_folder = os.path.join(code_location, 'TF-IDF+LinearSVC-Results')
...
# ==================== 4. CALIBRATE FOR PROBABILITIES ====================

print("\nCalibrating classifier for probability predictions...")
calibration_start = time.time()

best_svm = grid_search.best_estimator_
calibrated_svm = CalibratedClassifierCV(best_svm, cv=5, method='sigmoid')
...
time_metrics = {
    'Metric': ['Vectorization', 'Grid Search', 'Calibration', 'Inference (Test Set)', 'Total Execution'],
    'Time (seconds)': [vectorizer_time, gridsearch_time, calibration_time, inference_time, total_time],
    'Time (minutes)': [vectorizer_time/60, gridsearch_time/60, calibration_time/60, inference_time/60, total_time/60]
}
...
eval_df = pd.DataFrame({
    ...
    'probability_non_hate': 1 - y_pred_proba,
    'probability_hate': y_pred_proba
})
...
def plot_learning_curves(estimator, X, y, cv=10, train_sizes=np.linspace(0.1, 1.0, 10)):
    ...
    colors = ['#1f77b4', '#d62728', '#F18F01', '#C73E1D', '#3D5A80']  # Bleu et Rouge
    ...
    for idx, metric in enumerate(metrics):
        ...
        ax.plot(train_sizes_abs, test_mean, 'o-', color=colors[1], label='Cross-validation')
        
        ax.set_title(f'Courbe d\'apprentissage - {metric.upper()}')
        ax.set_xlabel('Taille du dataset d\'entraînement')  
        ax.set_ylabel(metric.upper())

        ax.set_yticks(np.arange(0.5, 1.05, 0.1))  # Pas de 0.1 de 0.5 à 1.0
        ax.set_ylim(0.5, 1.05)
        
        ax.legend(loc='best')
        ax.grid(True, alpha=0.3)
    
    # Plot Loss curve (using hinge loss approximation)
    ax_loss = axes[4]
    train_sizes_abs, train_scores_loss, test_scores_loss = learning_curve(
        estimator, X, y, cv=cv, scoring='neg_log_loss' if hasattr(estimator, 'predict_proba') else 'neg_mean_squared_error',
        train_sizes=train_sizes, n_jobs=-1
    )
    ...
    ax_loss.plot(train_sizes_abs, test_mean_loss, 'o-', color=colors[3], label='Cross-validation')
    ax_loss.set_title('Courbe d\'apprentissage - LOSS')
...
plot_learning_curves(svm_for_learning, X_train_tfidf, y_train)
...
# ==================== 9. SAVE MODEL FOR LATER USE (.pth format) ====================

print("\nSaving model for later use in interface...")

# Save as .pth (PyTorch format) and also as .pkl for compatibility
model_artifacts = {
...
joblib.dump(calibrated_svm, 'LinearSVC_model.pkl')
...
print("    5. learning_curves_data.csv - Learning curves raw data")
print("  📈 Visualizations:")
print("    6. learning_curves.png - Learning curves (styled as requested)")
print("    7. confusion_matrix.png - Confusion matrix visualization")
print("    8. roc_auc_curve.png - ROC AUC curve with metrics")
print("  🤖 Model Files:")
print("    9. LinearSVC_model.pkl - Trained model (joblib)")
print("    10. LinearSVC_model.pth - Trained model (PyTorch format)")
print("    11. vectorizer.pkl - TF-IDF vectorizer")
print("    12. model_metadata.pkl - Model metadata")
...
def predict_comment(text, model_path='LinearSVC_model.pth', vectorizer_path='vectorizer.pkl'):
    ...
    # Or use joblib if .pkl
    # vectorizer = joblib.load(vectorizer_path)
    # classifier = joblib.load('hate_speech_model.pkl')
...
for idx, row in sample_df.iterrows():
    ...
    print(f"   Prob (Hate): {row['probability_hate']:.4f} | Prob (Non-Hate): {row['probability_non_hate']:.4f}") 