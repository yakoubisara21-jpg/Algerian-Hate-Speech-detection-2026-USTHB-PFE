"""
RNN avec plongements AraVec pour la classification de texte - Version finale
...
class RNNTextClassifier(nn.Module):
    """
    Classifieur RNN unidirectionnel avec plongements AraVec GELÉS (pas de fine-tuning).
...
def _plot_kfold_metrics(df_full, df_plot, k_folds, save_path):
    ...
    plt.suptitle('Métriques validation croisée pour RNN avec AraVec',
...
def plot_roc_test(true_labels, probabilities, save_path):
    ...
    plt.title('ROC Curve - RNN avec AraVec', fontsize=14, fontweight='bold')
...
def train_final_model(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print("PLONGEMENTS: GELÉS (pas de fine-tuning)")
    print("=" * 70)
...
def plot_final_metrics(history, save_path):
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - RNN avec AraVec ',
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path):
    ...
    ax.set_title('Temps d\'exécution - RNN avec AraVec', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Matrice de confusion - RNN avec AraVec',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def save_vectorization(vocab, embedding_tensor, label_encoder, aravec_obj,
                        save_dir='DL Models/RNN with AraVec/vectorisation'):
...
def run_optuna(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print("RNN UNIDIRECTIONNEL - PLONGEMENTS GELÉS (pas de fine-tuning)")
    ...
    study = optuna.create_study(
        direction='maximize',
        study_name='rnn_aravec_f1_maximisation',
        sampler=sampler
    )
    ...
    results_dir = 'DL Models/RNN with AraVec/Résultats'
...
def main():
    ...
    print("=" * 70)
    print("RNN avec plongements AraVec (GELÉS) - Classification de texte avec OPTUNA")
    print("=" * 70)
    ...
    output_dir  = 'DL Models/RNN with AraVec'
