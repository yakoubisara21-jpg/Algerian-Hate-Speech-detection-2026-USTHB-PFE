"""
RNN avec plongements FastText (ENTRAÎNABLES) pour la classification de texte - Version finale
...
PLONGEMENTS ENTRAÎNABLES (FastText fine-tuning)
...
PLONGEMENTS FASTTEXT ARABES (ENTRAÎNABLES / FINE-TUNING)
"""
...
class FastTextEmbeddings:
    """Gestionnaire des plongements FastText arabes - ENTRAÎNABLES (fine-tuning)."""
    ...
    def get_embedding_tensor(self, vocab):
        ...
        print("NOTE: Les plongements seront ENTRAÎNABLES (fine-tuning / non gelés)")
...
class RNNTextClassifier(nn.Module):
    """
    Classifieur RNN unidirectionnel avec plongements FastText ENTRAÎNABLES (fine-tuning).
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim,
        ...
        if pretrained_embeddings is not None:
            ...
            self.embedding.weight.requires_grad = embedding_trainable  # ENTRAÎNABLE
            if embedding_trainable:
                print("Plongements FastText ENTRAÎNABLES (fine-tuning activé)")
            else:
                print("Plongements FastText NON ENTRAÎNABLES (gelés / frozen)")
        else:
            print("Plongements aléatoires (pas de pré-entraînement)")
...
def train_with_history(model, train_loader, val_loader, epochs, lr, weight_decay,
    ...
    for epoch in range(epochs):
        ...
        # Mise à jour du scheduler sur la perte de validation
...
def optuna_objective(trial):
    ...
    for fold, (train_idx, val_idx) in enumerate(
        ...
        model = RNNTextClassifier(
            ...
            embedding_trainable=True  # Plongements ENTRAÎNABLES
        ).to(device)
...
def _plot_kfold_metrics(df_full, df_plot, k_folds, save_path):
    ...
    plt.suptitle('Métriques validation croisée pour RNN avec DZ FastText ',
...
def plot_roc_test(true_labels, probabilities, save_path):
    ...
    plt.title('ROC Curve - RNN avec DZ FastText', fontsize=14, fontweight='bold')
...
def train_final_model(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print("PLONGEMENTS: ENTRAÎNABLES (fine-tuning)")
    print("=" * 70)
    ...
    print(f"Paramètres entraînables: {trainable_params:,} (embeddings inclus dans fine-tuning)")
    ...
    history = []
    print("\n" + "=" * 80)
    print(f"ENTRAÎNEMENT FINAL PENDANT {n_epochs} ÉPOQUES (fine-tuning actif)")
    print("=" * 80 + "\n")
    ...
    for name, param in model.named_parameters():
        if 'embedding' in name:
            print(f"  {name}: requires_grad = {param.requires_grad} (ENTRAÎNABLE)")
...
def plot_final_metrics(history, save_path):
    ...
    for i, (data, ylabel, title) in enumerate(series):
        axes[i].plot(epochs, data, 'b-o', linewidth=2, markersize=6,
                     label='Entraînement (fine-tuning)', alpha=0.8)
        axes[i].set_xlabel('Époque', fontsize=12)
        axes[i].set_ylabel(ylabel,   fontsize=12)
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - RNN avec DZ FastText',
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path):
    ...
    """
    labels = [f"Entraînement final\n({optimal_epochs} époques)\n(fine-tuning)", "Temps total exécution"]
    times  = [training_time, total_time]
    colors = ['#E65100', '#1565C0']
    ...
    ax.set_title('Temps d\'exécution - RNN avec DZ FastText', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Matrice de confusion - RNN avec DZ FastText',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def permutation_test(model, test_loader, criterion, n_permutations=999, threshold=0.5):
    ...
    ax.set_title('Test de randomisation - Distribution nulle (fine-tuning)',
                 fontsize=14, fontweight='bold')
    ax.legend()
...
def run_optuna(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print("RNN UNIDIRECTIONNEL - PLONGEMENTS ENTRAÎNABLES (fine-tuning)")
    ...
    study = optuna.create_study(
        direction='maximize',
        study_name='rnn_fasttext_finetuning_f1_maximisation',
        sampler=sampler
    )
    ...
    axes[0].set_title("Progrès de l'optimisation Optuna (F1 - fine-tuning)", fontsize=14, fontweight='bold')
    axes[0].legend()
...
def main():
    ...
    print("=" * 70)
    print("RNN avec plongements FastText (ENTRAÎNABLES) - Classification de texte avec OPTUNA")
    print("=" * 70)
    ...
    final_model, history, training_time = train_final_model(
        X_kfold, list(y_kfold), vocab, embedding_tensor,
        best_params, reduction_epochs, best_factor, optimal_epochs  # ← nouveaux args
    )
    ...
    if X_test and len(y_test) > 0:
        ...
        runtime_info = {
            ...
            'essais_optuna':               15,
            'plongements':                 'entrainables_finetuning'
        }
