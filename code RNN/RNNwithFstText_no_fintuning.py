"""
RNN avec plongements FastText (non ENTRAÎNABLES) pour la classification de texte - Version finale
...
def plot_roc_test(true_labels, probabilities, save_path):
    ...
    plt.title('ROC Curve - RNN avec FastText', fontsize=14, fontweight='bold')
...
def train_final_model(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print(f"Paramètres entraînables: {trainable_params:,} (embeddings GELÉS)")
    ...
    history = []
    print("\n" + "=" * 80)
    print(f"ENTRAÎNEMENT FINAL PENDANT {n_epochs} ÉPOQUES (embeddings gelés)")
    print("=" * 80 + "\n")
    ...
    for name, param in model.named_parameters():
        if 'embedding' in name:
            print(f"  {name}: requires_grad = {param.requires_grad} (GELÉ)")
...
def plot_final_metrics(history, save_path):
    ...
    for i, (data, ylabel, title) in enumerate(series):
        axes[i].plot(epochs, data, 'b-o', linewidth=2, markersize=6,
                     label='Entraînement (embeddings gelés)', alpha=0.8)
        axes[i].set_xlabel('Époque', fontsize=12)
        axes[i].set_ylabel(ylabel,   fontsize=12)
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - RNN avec FastText',
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path):
    ...
    ax.set_title('Temps d\'exécution - RNN avec FastText', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_title('Matrice de confusion - RNN avec FastText',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def run_optuna(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print("RNN UNIDIRECTIONNEL - PLONGEMENTS Non ENTRAÎNABLES ")
...
def main():
    ...
    print("=" * 70)
    print("RNN avec plongements FastText (NON ENTRAÎNABLES) - Classification de texte avec OPTUNA")
    print("=" * 70)
