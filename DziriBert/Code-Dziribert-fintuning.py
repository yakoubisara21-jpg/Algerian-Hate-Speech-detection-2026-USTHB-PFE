...
# Paramètres optimisés pour 19k échantillons
FIXED_EPOCHS   = 5          # 5 époques fixes
...
class DziriDataset(Dataset):
    """Dataset PyTorch pour DziriBERT"""
...
def load_data(kfold_path='/content/drive/MyDrive/BigData_11_2026/data/kfoldsdata.csv',
    ...
    # FIX: Always return numpy array for consistency
...
def build_model_and_optimizer(num_classes, epochs, steps_per_epoch):
    """Construire le modèle avec optimizer et scheduler"""
    ...
    print("Toutes les couches sont entraînables")
...
def run_kfold(all_texts, all_labels, label_encoder, results_dir, timestamp):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splits):
        ...
        train_dataset = DziriDataset(train_texts_fold, train_labels_fold, tokenizer)
        val_dataset = DziriDataset(val_texts_fold, val_labels_fold, tokenizer)
    ...
    for metric in metrics_names:
        ...
        for e in range(FIXED_EPOCHS):
            ...
            for p in range(len(fold_histories)):
                values.append(fold_histories[p][e].get(metric, float('nan')))
    ...
    auc_at_optimal = []
    for p in range(len(fold_histories)):
        auc_at_optimal.append(fold_histories[p][optimal_epoch-1].get('val_auc', float('nan')))
...
def _plot_kfold_metrics(df_plot, k_folds, title_suffix, save_path):
    ...
    for i, (train_col, val_col, ylabel, title_label) in enumerate(metrics_config):
        if train_col and train_col in df_plot.columns:
            axes[i].plot(epochs, df_plot[train_col], 'b-o', label='Entraînement', linewidth=2)

        if val_col and val_col in df_plot.columns:
            axes[i].plot(epochs, df_plot[val_col], 'r-s', label='Validation', linewidth=2)
    ...
    plt.suptitle(f'Métriques de la validation croisée{title_suffix} - DziriBERT avec affinage complet (Full fine-tuning)')
    plt.tight_layout()
...
def train_final_model(train_texts, train_labels, label_encoder, n_epochs):
    ...
    dataset = DziriDataset(train_texts, train_labels, tokenizer)
...
def plot_final_metrics(history, save_path):
    ...
    axes[5].set_visible(False)
    plt.suptitle("Métriques d'entraînement final - DziriBERT avec affinage complet (Full fine-tuning)")
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path, save_dir, timestamp):
    ...
    ax.set_title("Temps d'exécution - DziriBERT avec affinage complet (Full fine-tuning)")
...
def compute_and_plot_test_auc(true_labels, probabilities, label_encoder, results_dir, timestamp):
    """Calculer et tracer l'AUC sur le test"""
    ...
    if num_classes == 2:
        ...
        ax.plot(fpr, tpr, 'b-', linewidth=2, label=f'Courbe ROC (AUC = {auc_val:.4f})')
        ...
        ax.set_title('Courbe ROC - DziriBERT avec affinage complet (Full fine-tuning)')
        ax.legend()
        ax.grid(alpha=0.3)
        plt.tight_layout()
        ...
        auc_val = roc_auc_score(true, prob_matrix, multi_class='ovr', average='weighted')
        print(f"AUC pondéré: {auc_val:.4f}")
        ...
        for i, (cls_name, color) in enumerate(zip(label_encoder.classes_, colors)):
            ...
            ax.plot(fpr_i, tpr_i, color=color, label=f'Classe {cls_name} (AUC={auc_i:.3f})')

        ax.plot([0, 1], [0, 1], 'k--', label='Classifieur aléatoire')
        ax.set_xlabel('Taux de faux positifs')
        ax.set_ylabel('Taux de vrais positifs')
        ax.set_title(f'Courbes ROC One-vs-Rest - DziriBERT fine-tuné\nAUC pondéré = {auc_val:.4f}')
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_ylabel('Réel')
    ax.set_title('Matrice de confusion - DziriBERT avec affinage complet (Full fine-tuning)')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def permutation_test(model, tokenizer, test_texts, test_labels, label_encoder,
    ...
    test_dataset = DziriDataset(test_texts, test_labels, tokenizer)
    ...
    if p_value < 0.001:
        ...
        significance = "n.s. non significatif"

    print(f"\np-valeur: {p_value:.4f} - {significance}")

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.hist(null_dist, bins=30, alpha=0.7, color='gray', edgecolor='black', label='Distribution nulle')
    ax.axvline(obs_acc, color='red', linewidth=2, label=f'Observée (p={p_value:.4f})')
    ax.axvline(null_dist.mean(), color='blue', linestyle='--', label=f'Moyenne = {null_dist.mean():.3f}')
    ax.set_xlabel('Exactitude')
    ax.set_ylabel('Fréquence')
    ax.set_title('Test de permutation - DziriBERT avec affinage complet (Full fine-tuning)')
    ax.legend()
    ax.grid(alpha=0.3)
    plt.tight_layout()
...
def save_evaluation_results(test_texts, true_labels, predictions, probabilities,
                             label_encoder, output_path, num_classes):
    ...
    print(f"Résultats complets sauvegardés: {output_path}")
    return df
...
def main():
    ...
    print("="*70)
    print("DziriBERT Fine-tuning - Classification de texte")
    print(f"K-Folds K={K_FOLDS}, {FIXED_EPOCHS} epoques")
    print(f"LR={LEARNING_RATE}, Dropout={DROPOUT}, WD={WEIGHT_DECAY}")
    print("="*70)

    data_dir = '/content/drive/MyDrive/BigData_11_2026/data'
    output_dir = '/content/drive/MyDrive/BigData_11_2026/DziriBert'
    ...
    # Check if test data exists and has samples
    if X_test and len(y_test) > 0:
        ...
        test_loss, test_acc, test_prec, test_rec, test_f1, predictions, true_labels, probabilities = evaluate(
        ...
        save_classification_report(true_labels, predictions, label_encoder, results_dir, timestamp)

        auc_test = compute_and_plot_test_auc(true_labels, probabilities, label_encoder, results_dir, timestamp)
        ...
        save_evaluation_results(
                                ...
                                f"{results_dir}/resultats_evaluation.csv",
                                num_classes
        )

        metrics = compute_metrics(true_labels, predictions, is_binary)
        pd.DataFrame([metrics]).to_csv(f"{results_dir}/metriques_{timestamp}.csv", index=False)

        plot_confusion_matrix(true_labels, predictions, label_encoder,
                             f"{results_dir}/confusion_matrix_{timestamp}.png")

        perm_results = permutation_test(final_model, tokenizer, X_test, y_test, label_encoder)
        perm_results['figure'].savefig(f"{results_dir}/permutation_{timestamp}.png", dpi=300)
        plt.close()
        ...
        pd.DataFrame([runtime_info]).to_csv(f"{results_dir}/execution_{timestamp}.csv", index=False)
        ...
        print(f"p-valeur permutation: {perm_results['p_valeur']:.4f}")
        print("\n" + "="*70)
        print("TERMINE!")
        print("="*70)

        return final_model, tokenizer, label_encoder, metrics, total_time, {'auc_cv': mean_auc_cv, 'auc_test': auc_test}
    else:
