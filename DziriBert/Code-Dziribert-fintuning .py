...
def _plot_kfold_metrics(df_plot, k_folds, title_suffix, save_path):
    ...
    metrics_config = [
        ...
        ('val_auc_mean', None, 'AUC', 'AUC (Validation)'),
    ]
    ...
    for i, (train_col, val_col, ylabel, title_label) in enumerate(metrics_config):
        ...
        elif train_col is None and val_col and val_col in df_plot.columns:
            axes[i].plot(epochs, df_plot[val_col], 'r-s', label='Validation', linewidth=2)

        axes[i].set_xlabel('Époque')
        axes[i].set_ylabel(ylabel)
        axes[i].set_title(f'{title_label} (moy sur {k_folds} folds)')
        axes[i].legend()
        axes[i].grid(alpha=0.3)

    plt.suptitle(f'Métriques de la validation croisée{title_suffix} - DziriBERT fine-tuné')
    plt.tight_layout()
...
def plot_final_metrics(history, save_path):
    ...
    plt.suptitle("Métriques d'entraînement final - DziriBERT fine-tuné")
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path, save_dir, timestamp):
    ...
    ax.set_title("Temps d'exécution - DziriBERT fine-tuné")
...
def compute_and_plot_test_auc(true_labels, probabilities, label_encoder, results_dir, timestamp):
    ...
    if num_classes == 2:
        ...
        ax.set_title('Courbe ROC - DziriBERT fine-tuné')
        ax.legend()
        ax.grid(alpha=0.3)
        plt.tight_layout()
...
def plot_confusion_matrix(true_labels, predictions, label_encoder, save_path):
    ...
    ax.set_ylabel('Réel')
    ax.set_title('Matrice de confusion - DziriBERT fine-tuné')
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def permutation_test(model, tokenizer, test_texts, test_labels, label_encoder,
    ...
    ax.set_title('Test de permutation - DziriBERT fine-tuné')
    ax.legend()
    ax.grid(alpha=0.3)
    plt.tight_layout()
