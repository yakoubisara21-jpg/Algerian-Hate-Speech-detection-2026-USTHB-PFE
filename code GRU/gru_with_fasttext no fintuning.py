"""
GRU avec plongements FastText (non entraînables) pour la classification de texte - Version finale
...
class FastTextEmbeddings:
    """Gestionnaire des plongements FastText arabes - NON ENTRAÎNABLES."""
    ...
    def get_embedding_tensor(self, vocab):
        ...
        print("NOTE: Les plongements seront NON ENTRAÎNABLES (pas de fine-tuning) ")
...
class GRUClassifier(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim,
        ...
        if pretrained_embeddings is not None:
            ...
            print("Plongements FastText NON ENTRAÎNABLES ")
...
def train_with_history(model, train_loader, val_loader, epochs, lr, weight_decay,
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='min', factor=factor, patience=patience , min_lr=1e-7  # patience=0, WATCHES val_loss
    )

    history = []
    val_probs_per_epoch = []
    reduction_epochs_fold = []  # NEW
    prev_lr = lr                # NEW
    ...
    for epoch in range(epochs):
        ...
        scheduler.step(val_loss)  # CHANGED: was train_loss

        current_lr = optimizer.param_groups[0]['lr']  # NEW
        if current_lr < prev_lr:                       # NEW
            reduction_epochs_fold.append(epoch)        # NEW
        prev_lr = current_lr                           # NEW
    ...
    return history, val_probs_per_epoch, reduction_epochs_fold  # CHANGED return signature
...
def optuna_objective(trial):
    ...
    factor           = trial.suggest_float('factor',           0.1,  0.9, step=0.05)  # REPLACES scheduler_factor+patience
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(global_train_texts, global_train_labels)):
        ...
        model = GRUClassifier(
            vocab_size=len(global_vocab), embedding_dim=300,
            hidden_dim=hidden_dim, output_dim=1, n_layers=n_layers,
            dropout_pre_gru=dropout_pre_gru, dropout_gru=dropout_gru,
            dropout_post_gru=dropout_post_gru,
            pretrained_embeddings=global_embedding_tensor
        ).to(device)

        history, _, _ = train_with_history(  # CHANGED: 3 return values now
            model, train_loader, val_loader, FIXED_EPOCHS,
            learning_rate, weight_decay, factor,patience, clip_norm  # REMOVED scheduler_patience
        )
...
def run_kfold_best_params(fold_histories, results_dir, timestamp):
    """Agréger les métriques K-Fold et générer les graphiques."""
    print("\n" + "=" * 70)
...
def _plot_kfold_metrics(df_full, df_plot, k_folds, save_path):
    ...
    plt.suptitle('Métriques validation croisée pour GRU avec FastText',
...
def plot_roc_test(true_labels, probabilities, save_path):
    ...
    plt.title('ROC Curve - GRU avec FastText', fontsize=14, fontweight='bold')
...
def plot_final_metrics(history, save_path):
    ...
    axes[5].set_visible(False)
    plt.suptitle('Métriques d\'entraînement final - GRU avec FastText',
...
def plot_time_chart(training_time, total_time, optimal_epochs, save_path):
    ...
    ax.set_title('Temps d\'exécution - GRU avec FastText', fontsize=14, fontweight='bold')
...
def save_vectorization(vocab, embedding_tensor, label_encoder, fasttext_obj,
                        save_dir='DL Models/GRU with FastText/vectorisation'):
...
def run_optuna(train_texts, train_labels, vocab, embedding_tensor,
    ...
    print("GRU UNIDIRECTIONNEL - PLONGEMENTS NON ENTRAÎNABLES (FastText)")
    ...
    study = optuna.create_study(
        direction='maximize',
        study_name='gru_fasttext_f1_maximisation',
        sampler=sampler
    )
    ...
    results_dir = 'DL Models/GRU with FastText/Résultats'
...
def main():
    ...
    print("=" * 70)
    print("GRU avec plongements FastText (non entraînables) - Classification de texte avec OPTUNA")
    print("=" * 70)
    ...
    print(f"\nÉpoque optimale: {optimal_epochs}")

    # Phase 3
    final_model, history, training_time = train_final_model(
        X_kfold, list(y_kfold), vocab, embedding_tensor,
        best_params, reduction_epochs, best_factor, optimal_epochs  # NEW ARGS
    )
